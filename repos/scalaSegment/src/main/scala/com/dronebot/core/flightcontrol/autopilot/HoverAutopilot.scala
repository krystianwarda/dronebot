...
object HoverAutopilot {
  ...
  def streamWithDynamicTarget[F[_]: Async](
                                            telemetryWithSeg: Stream[F, (DroneTelemetry, Any)],
                                            ranges: JoystickRanges,
                                            targetZFn: (DroneTelemetry, Any) => Double
                                          ): Stream[F, ControllerState] = {

    type Seg = Any // kept generic; planner can pass `FlightSegment` here
    ...
    telemetryWithSeg
      .evalMapAccumulate(init) { (st, in) =>
        ...
        val targetZ = targetZFn(t, seg.asInstanceOf[Seg])
        val altErr = targetZ - t.position.z
        val (altOut, alt1) = st.alt.step(altErr, dt)
        val throttleNorm01 = clamp(0.5 + altOut, 0.0, 1.0)
        val throttleNormSigned = (throttleNorm01 - 0.5) * 2.0
