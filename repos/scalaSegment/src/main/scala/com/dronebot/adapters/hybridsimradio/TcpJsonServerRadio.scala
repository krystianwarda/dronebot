...
object TcpJsonServerRadio {
  ...
  def stream[F[_]: Async](bindHost: String, bindPort: Int): Stream[F, ControllerState] = {
    ...
    val bindLog =
      Stream.eval(Async[F].delay(println(s"[TCP-SERVER] binding on $bindHost:$bindPort")))

    val server =
      Network[F].server(address = Some(host), port = Some(port)).flatMap { socket =>
        val announce =
          Stream.eval(
            Async[F].flatMap(socket.remoteAddress) { ra =>
              Async[F].delay(println(s"[TCP-SERVER] client connected: $ra"))
            }
          )
